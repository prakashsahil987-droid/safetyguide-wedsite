...
export default function BankingPage() {
  const cat = getCategoryBySlug("banking");
  if (!cat) notFound();
  const articles = getArticlesByCategory("banking");
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
      ...
      <h1 className="text-3xl md:text-4xl text-ink mb-3" style={{ fontFamily: "var(--font-display)" }}>
        {cat.title}
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {articles.map((article) => (
          <ArticleCard key={article.slug} article={article} categorySlug="banking" />
        ))}
      </div>
    </div>
  );
