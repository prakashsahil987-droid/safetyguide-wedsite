...
export default function TravelArticlePage({ params }: Props) {
  const article = getArticle(params.slug);
  if (!article || article.category !== "travel") notFound();
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      <nav className="flex items-center gap-2 text-sm text-ink-muted mb-8">
        ...
        <span>/</span>
        <Link href="/travel" className="hover:text-ink transition-colors">{cat?.title}</Link>
        <span>/</span>
      ...
      <div className="mt-10">
        <Link href="/travel" className="inline-flex items-center gap-2 text-trust hover:text-trust-dark text-sm font-medium transition-colors">
          <ArrowLeft size={15} /> Back to Travel Safety
        </Link>
      </div>
    </div>
  );
